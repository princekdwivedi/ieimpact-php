...
function redirectViewPageTo(memberId,orderId,flag)
{
	window.location.href="<?php echo SITE_URL_EMPLOYEES;?>/accept-orders-behalf-employee.php?orderId="+orderId+"&customerId="+memberId+"&"+flag;
}
</script>
	<table width="100%" align="center" border="0" cellspacing="0" cellspacing="0">
		...
		<tr bgcolor="#373737" height="20">
			...
			<td width="12%" class="smalltext8"><b>Order Type</b></td>
			<td width="8%" class="smalltext8"><b>Date</b></td>
			<td width="17%" class="smalltext8"><b>Accepted By</b></td>
			<td width="17%" class="smalltext8"><b>Qa By</b></td>
			<td width="6%" class="smalltext8"><b>Cus. Rate</b></td>
			<td class="smalltext8"><b>Message</b></td>
		</tr>
		<?php
			$orderAcceptedHrs			=	"";
			$orderQaHrs					=	"";
			$query						=	"SELECT members_orders.orderId,orderAddress,orderType,acceptedBy,rateGiven,orderAddedOn,orderAddedOn,memberRateMsg,timeSpentEmployee,timeSpentQa,qaDoneByName,acceeptedByName,isRateCountingEmployeeSide FROM members_orders INNER JOIN members_orders_reply ON members_orders.orderId=members_orders_reply.orderId WHERE members_orders.memberId=$customerId AND status IN (2,4,5) ORDER BY orderAddedOn DESC LIMIT 5";
...
<tr height="26" <?php echo $bgColor;?>>
	...
	<td class="smalltext6" valign="top">
		<?php
			if(!empty($t_rateGiven) && $isRateCountingEmployeeSide == "yes")
			{
		?>
...
<form name="acceptOrder" action="" method="POST" onsubmit="return validAccept();">
	...
	<table cellpadding="2" cellspacing="2" width='98%'align="center" border='0'>
		<tr>
			...
			<td width="17%" class="smalltext2"><b>Todays/Yesterday orders</b></td>
		...
		<?php
			...
			if(mysqli_num_rows($result))
			{
				while($row						=  mysqli_fetch_assoc($result))
				{
					...
					if(count($a_totalOrdersAccepted) > 0 && array_key_exists($employeeId,$a_totalOrdersAccepted)){
