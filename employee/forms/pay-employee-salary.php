...
function calculateTotalMoney()
{
	...
	lineMoney			=	document.getElementById('lineMoney').value;
	if(fixedSal != "")
	{
	...
	if(lineMoney != "")
	{
		lineMoney = Math.abs (lineMoney)

		total		= fixedSal+lineMoney;
...
<form name="paySalary" action=""  method="POST" onsubmit="return validPay();">
	<table cellpadding="2" cellspacing="2" width='100%'align="center" border='0'>
		...
		<tr>
			<td class="textstyle">
				<b>Money From Lines</b>
			</td>
			<td class="textstyle1">
				:
			</td>
			<td class="textstyle1">
				<input type="text" name="totalMoney" value="<?php echo $totalMoney;?>" size="10" maxlength="10" onKeyPress="return checkForNumberPoints()" id="lineMoney" onBlur="calculateTotalMoney();">
			</td>
		</tr>
		<tr>
			<td class="textstyle">
				<b>TOTAL FIXED+LINES SALARY</b>
			</td>
			<td class="textstyle1">
				:
			</td>
			<td class="textstyle1">
				<div id="totalMoney" class="textstyle1"><b><?php echo $totalLineFixedMoney;?></b></div>
				<input type="hidden" name="totalSalaryReceived" value="<?php echo $totalLineFixedMoney;?>" readonly  id="salaryReceived" onBlur="calculateSalaryToGive();">
			</td>
		</tr>
		<?php
			if($employeeType == 2)
