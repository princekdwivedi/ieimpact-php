<?php
	...
	$email				=	"";
	$loginId			=	0;
	$formTopText		=	"Enter Employee Registration Email To Get Password";
	...
	if(isset($_REQUEST['formSubmitted']))
	{
		...
		if($dataValid)
		{
			if($result				=	$employeeObj->getForgotPasswordEmail($email,0))
			{
				...
				$email				=	$row['email'];
				$name				=	$row['fullName'];
				$newPasswordCode	=	substr(md5((float) microtime(true) + (int) rand() + (int) date('s')), 0, 30);
				$newPasswordCode	=	$loginId."emp".$newPasswordCode;

				dbQuery("UPDATE employee_details SET isRequestForPassword=1,newPasswordCode='$newPasswordCode' WHERE employeeId=$loginId");


				$from			=	"support@ieimpact.com";
				$fromName		=	"ieIMPACT";
				$to				=	$email; 
				$mailSubject	=	"ieIMPACT - Password Assistance";
				$templateId		=	TEMPLATE_SENDING_EMPLOYEE_PASSWORD;

				$changePasswordLink		=	"<a href=\"".SITE_URL_EMPLOYEES. "/reset-password.php?code=".$newPasswordCode."\" target=\"_blank\">".SITE_URL_EMPLOYEES. "/reset-password.php?code=".$newPasswordCode ."</a>";
				
				//Pass template parameters 1st templateFile, then the replacing variables.//
				$a_templateData	=	array("{name}"=>$name,"{changePasswordLink}"=>$changePasswordLink);
				sendTemplateMail($from, $fromName, $to, $mailSubject, $templateId, $a_templateData);

				$employeeObj->trackEmployeePasswordSecurityTracking($loginId,0,1,$requestMadeThroughEmail,$requestMadeThroughId,$email,$requestTokenFromIP,$requestTokenIpCity,$requestTokenIpRegion,$requestTokenIpZipCode,$requestTokenIpCountry,$requestTokenIpISP);
			...
			if($isAjax) {
				...
				echo json_encode(array(
					'success' => true,
					'message' => 'We are sending you an email with reset password link. Simply click the link to reset your password. If you do not find the email in your inbox, please check your spam filter or bulk email folder.'
				));
				exit;
			}
		}
	...
	if($isAjax && !isset($_REQUEST['formSubmitted'])) {
		include(SITE_ROOT_EMPLOYEES . "/forms/modal-forgot-password.php");
		exit;
	}
...
<body>
	<?php
		if(isset($_REQUEST['formSubmitted']) && $dataValid) {
			...
			echo '<p style="color: #666; line-height: 1.6;">We are sending you an email with reset password link. Simply click the link to reset your password. If you do not find the email in your inbox, please check your spam filter or bulk email folder.</p>';
