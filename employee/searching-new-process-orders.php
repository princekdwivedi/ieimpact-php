<?php
	...
	if(isset($_SESSION['employeeId'])){
		...
		if($isEnglish  == 1){
			...
			$query					=	"SELECT orderAddress FROM members_orders WHERE orderId > ".MAX_SEARCH_MEMBER_ORDERID." AND orderAddress LIKE '%$searchOrder%' AND isVirtualDeleted=0 AND isDeleted=0 AND status IN (0,1,3)";
