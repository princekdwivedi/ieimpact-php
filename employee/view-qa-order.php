<?php
	...
	if(isset($_GET['orderId']) && isset($_GET['customerId']))
	{
		...
		$qaAcceptedByEmployee	=	$employeeObj->getSingleQueryResult("SELECT qaAcceptedBy FROM members_orders_reply WHERE isQaAccepted=1 AND qaAcceptedBy <> 0 AND orderId=$orderId","qaAcceptedBy");
	...
	if(isset($_GET['doneQa']) && $_GET['doneQa'] == 1)
	{
		...
		else
		{
			...
			if(isset($_GET['replyId']))
			{
				...
				if(!empty($replyId))
				{
					$orderObj->markOrderQaDone($orderId,$replyId,$customerId,$s_employeeId,$s_employeeName);
				}
...
<?php
	...
	$totalChecklistOrderTotal		=	$employeeObj->getSingleQueryResult("SELECT checklistOrderTotal FROM members WHERE memberId=$customerId","checklistOrderTotal");
	$totalPoorAverageChecklistTotal	=	$employeeObj->getSingleQueryResult("SELECT poorAverageChecklistTotal FROM employee_details WHERE employeeId=$acceptedBy","poorAverageChecklistTotal");
	...
	if(isset($_REQUEST['formSubmitted']))
	{
		...
		{	
			...
			if($isAllowedRewards == 1 && $postOrderCost >= MINIMUM_REWRADS_ORDER_PRICE){
			...
			if($isRushOrder	!=	2 && $isRushOrder	!=	3){
				
				$completedOrderTimeTaken	=	$employeeObj->getSingleQueryResult("SELECT beforeAfterTimingMin FROM members_orders WHERE orderId > ".MAX_SEARCH_MEMBER_ORDERID." AND orderId=$orderId AND isCompletedOnTime=2 AND memberId=$customerId AND beforeAfterTimingMin > 120 AND isRushOrder=$isRushOrder","beforeAfterTimingMin");
				...
				if(!empty($completedOrderTimeTaken)){
					...
					if(!empty($lacarteOrderPrice)){
						$n_postOrderCost            =  $lacarteOrderPrice;
					}
					else{						

						$a_allOrdersPrice			=	$memberObj->getCustomersAllOrderPrice($customerId,$nowDateIndia); 

						$n_postOrderCost			=	$a_allOrdersPrice[2];
						if($orderType				==	19){
							$n_postOrderCost		=	$a_allOrdersPrice[4];
						}
						elseif($orderType			==	20){
							$n_postOrderCost		=	$a_allOrdersPrice[6];
						}
					}					

					$isRevertedDueToEta			=	1;
					$extraAdjustColumn			=	",cost='$n_postOrderCost'";

					$existingPostOrderCost		=	$postOrderCost;
					if(empty($existingPostOrderCost)){
						$existingPostOrderCost	=	0;
					}

					$transactionsMoney			=	$existingPostOrderCost-$n_postOrderCost;
					...
					if($isPaidThroughWallet			==  "yes" && !empty($walletAccountId))
					{
						...
						if(!empty($transactionsMoney) && $existingPostOrderCost > $n_postOrderCost){
					...
					if(!empty($prepaidOrderPrice)){
						$exCol			   =	",prepaidOrderPrice='$n_postOrderCost'";
						$prepaidOrderPrice = $n_postOrderCost;
					}
					dbQuery("UPDATE members_orders SET postOrderCost='$n_postOrderCost',rushOrderExtraMoney='0',isExceededEtaAdjust=1".$exCol.$extraAdjustColumn." WHERE orderId=$orderId AND memberId=$customerId");
				}
			}
			//die("KASE22");
			////////////////////////////////////////////////////////////////////////////////////////
			...
			$totalFreeOrdersLeft=	$employeeObj->getSingleQueryResult("SELECT customerFreeOrders FROM members WHERE memberId=$customerId","customerFreeOrders");
			////////////////////////////////////////////////////////////////////////////////////////////
			////////////////////////////////////////////////////////////////////////////////////////////
			...
			elseif($isPrepaidOrder == 0 && !empty($stripeChargeId) && $paymentGateway == 'Stripe')
			{
				...
				if($isChangedPrice == 1 && $ignorePrepaidCapture == 1)
				{
					...
					$order_cost_money			=	$employeeObj->getSingleQueryResult("SELECT postOrderCost FROM members_orders WHERE orderId > ".MAX_SEARCH_MEMBER_ORDERID." AND orderId=$orderId AND memberId=$customerId","postOrderCost");
					$paymentType				=	2;
					$statusText					=	"Successfully Advanced Payment Made Through Credit Card";
					if($prepiadPaymentThrough	==	"creditcard")
					{
						...
						if(!empty($usingStripeAccountId)){
							$transactionCardType	=	$employeeObj->getSingleQueryResult("SELECT cardType FROM auto_remember_stripe_account_details WHERE cardType <> '' AND accountId=$usingStripeAccountId AND memberId=$customerId","cardType");
				...
				else
				{
					...
					else 
					{
						...
						$cardLastDigits 			=	$employeeObj->getSingleQueryResult("SELECT cardLastDigits FROM auto_remember_stripe_account_details WHERE accountId=$usingStripeAccountId AND memberId=$customerId", "cardLastDigits");

						$chargedFromType 	=   " (Credit card ".$cardLastDigits.")";
			...
			elseif($isPrepaidOrder == 0 && !empty($prepaidTransactionId) && empty($advancedPaymentId))
			{
				if($isChangedPrice == 1 && $ignorePrepaidCapture == 1)
				{
					...
					if($prepiadPaymentThrough	==	"creditcard")
					{
						...
						if(!empty($customerPaymentProfileId)){
							$transactionCardType	=	$employeeObj->getSingleQueryResult("SELECT cardType FROM auto_remember_account_details WHERE cardType <> '' AND customerPaymentProfileId=$customerPaymentProfileId AND memberId=$customerId","cardType");	
							if($transactionCardType == "American Express"){
								$isAmaxTransactions	=	1;
							}
						}
				...
				else
				{
					...
					else
					{
						...
						$chargeMoneyWith 		=	$order_cost_money;

						$order_cost_money		=	displayMoneyExpo($order_cost_money);
						$is_marked_as_postpaid	=	false;	

						
						//////////////////////// NEW CODE WITH NEW FUNCTION //////////////
						try{
							...
							$cardLastDigits 			=	$employeeObj->getSingleQueryResult("SELECT cardLastDigits FROM auto_remember_account_details WHERE customerPaymentProfileId=$customerPaymentProfileId AND memberId=$customerId AND customerProfileId=$customerProfileId", "cardLastDigits");

							$chargedFromType 	=   " (Credit card ".$cardLastDigits.")";

		
							$chargeAuth = $objAuthorizeAPI->chargedAuthCC($customerProfileId, $customerPaymentProfileId, $chargeMoneyWith, $prepaidTransactionId);
							//pr($chargeAuth);
							$chargeAuth = json_decode($chargeAuth,true);
							//pr($chargeAuth);

							//die();

							$email_authorize_response = @recursive_implode($chargeAuth);
							$addedText		=	"QA Done Order - ".$orderAddress;
							$commonClass	=    $commonClass->trackAuthorizeResponse($chargeAuth,'Authorize.net',$orderId,$customerId,$addedText);

							if(array_key_exists('success',$chargeAuth) && $chargeAuth['success'] == 1 && array_key_exists('paymentFlag',$chargeAuth) && $chargeAuth['paymentFlag'] == 1){
								...
								/////////////////////IF FAILED PAYMENT ///////////
								$is_marked_as_postpaid	=	true;
								$errorCode				=	$chargeAuth['errorCode'];									
								$cardAccountErrorMsg    = htmlspecialchars($authResponse['message'])."&nbsp;(Error Code - ".$errorCode.")<br />Gateway Response : ".$email_authorize_response."<br />Profile Id - ".$customerProfileId." - paymentprofileid - ".$customerPaymentProfileId." - money - ".$chargeMoneyWith." - transaction id - ".$prepaidTransactionId;
							}
						}
						catch(Exception $e){
							$is_marked_as_postpaid	=	true;
							$cardAccountErrorMsg	=	$e->getMessage();
							$cardAccountErrorMsg    =   $cardAccountErrorMsg."<br />Profile Id - ".$customerProfileId." - paymentprofileid - ".$customerPaymentProfileId." - money - ".$chargeMoneyWith." - transaction id - ".$prepaidTransactionId;	
						}	
			...
			else
			{
				if($isChangedPrice	==	0 && $isPaidThroughWallet == "no" && $isRevertedDueToEta == 0)
				{
					...
					$memberObj->updateMemberOrderPriceNew($orderType,$orderId,$customerId,$orderAddedISTOn);
				}
			}		
			...
			if($hasReceiveEmails		== 0)
			{
				...
				$a_templateData			=	array("{name}"=>$customerName,"{orderNo}"=>$orderAddress,"{orderType}"=>$orderText,"{instructions}"=>$replyInstructions,"{needFeedBackMessage}"=>$needFeedBackMessage,"{showFilesNameInEmail}"=>$showFilesNameInEmail,"{trackEmailImage}"=>$trackEmailImage,"{sendingFileAttachmentMsg}"=>$sendingFileAttachmentMsg,"{referFriendLink}"=>$referFriendLink,"{excellentLink}"=>$excellentLink,"{goodLink}"=>$goodLink,"{fairLink}"=>$fairLink,"{poorLink}"=>$poorLink,"{awfulLink}"=>$awfulLink,"{walletAmountEmail}"=>$walletAmountEmail,"{priceCharged}"=>$orderExactCost);
			...
			if(!empty($a_managerEmails))
			{
				...
				if(isset($calculateCustomerAverageTime) && !empty($calculateCustomerAverageTime)){
					$acceptedByName     =  $acceptedByName." (Spent:".$calculateCustomerAverageTime." Minutes)";
				}

				$qaDoneBy 				=	$s_employeeName." (Spent:".$timeSpentQa." Minutes)";
					
				$a_templateData			=	array("{managerName}"=>"Manager","{instructions}"=>$replyInstructions,"{orderNo}"=>$orderAddress,"{orderType}"=>$orderText,"{customerName}"=>$customerName,"{acceptedBy}"=>$acceptedByName,"{qaDoneBy}"=>$qaDoneBy,"{showFilesNameInEmail}"=>$showFilesNameInEmail,"{trackEmailImage}"=>$trackEmailImage,"{sendingFileAttachmentMsg}"=>$sendingFileAttachmentMsg,"{excellentLink}"=>$excellentLink,"{goodLink}"=>$goodLink,"{fairLink}"=>$fairLink,"{poorLink}"=>$poorLink,"{awfulLink}"=>$awfulLink,"{walletAmountEmail}"=>$walletAmountEmail,"{priceCharged}"=>$orderExactCost);
			...
			//////////////////// PUTTING THE ORDER IN ORDER TRACK LIST ////////////////////////////
		     $orderObj->addOrderTracker($s_employeeId,$orderId,$orderAddress,'Employee marked as QA done this this order','EMPLOYEE_QA_DONE_ORDER');
		    ////////////////////////////////////////////////////////////////////////////////////////
		    ////////////////////////////////////////////////////////////////////////////////////////
