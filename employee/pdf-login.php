<?php
	...
	$loginFormHeadingText		=	"PDF Employee Login Area";
	...
	if(isset($_GET['error']))
	{
		...
		elseif($error	==	8)
		{
			$loginError	=	"Seems like you are trying to open an invalid page.";
		}
	}
			...
			if(mysqli_num_rows($result)){
			    ...
			    if(!empty($lastLoginDate) && $lastLoginDate != "0000-00-00"){
			    	...
			    	if($currentDate > $lastLoginDate && $employeeType == 1){
			    		$dateDifference     =   $currentDate - $lastLoginDate;
			    		$differentDays 		=	floor($dateDifference / (60*60*24) );
			    		if($differentDays >= 15){
				...
				if($t_hasOutsideLoginAccess == 1 && $t_bypassOtp == 0 && $t_isActive == 1 && $t_hasPdfAccess == 1 && !empty($a_officeIPAddress) && !in_array($employeeLoginFromIPUptoTwo,$a_officeIPAddressUptoTwo) && $isRequiredOtp == 0){
					
					$_SESSION['isNeededOtp'] =  1;
					$isSendOtp				 =	1;
					

					$isAlreadySent	    =	$employeeObj->getSingleQueryResult("SELECT isOtpRequired FROM employee_details WHERE employeeId=$loginId AND codeExpireOn >= '$currentDateTime' AND isOtpRequired=1","isOtpRequired");
					...
					ob_clean();
					header("Location: ".SITE_URL_EMPLOYEES."/pdf-login.php?error=6");
					exit();
		...
		if(!empty($securityToken) && !empty($loginId) && $displayCaptchaError == 0 && $validator)
		{
			...
			if($employeeType			 ==	1)
			{
				...
				if($securityToken		!= $employeeOwnSecurityCode)
				{
					...
					{
						ob_clean();
						header("Location: ".SITE_URL_EMPLOYEES."/pdf-login.php?error=5&".$checkingFailCaptcha."=".$checkingFailCaptchaCode);
						exit();
					}
					...
					elseif($attemptedTimes > 6)
					{
						...
						ob_clean();
						header("Location: ".SITE_URL_EMPLOYEES."/pdf-login.php?error=4&".$checkingFailCaptcha."=".$checkingFailCaptchaCode);
						exit();
					}
				}
				else
		...
		{			
			...
			if(!empty($otpErrorMessage)){
				...
				{					
					...
					$result	    =   $employeeObj->PDFEmployeeLoginWithNewPassword($loginId,$password,$loginEmail,$loginIpCountry);
					...
					if($result)
					{
						...
						if(!empty($isForcedToDeactivateAccount)){
							...
							ob_clean();
							header("Location: ".SITE_URL_EMPLOYEES."/pdf-login.php?error=7");
							exit();
						...
						if(is_numeric($result))
						{
							...
							$_SESSION['FACEBOOK_LOGIN_PROCESSED_ON'] = $start_time_stamp;
						...
						else
						{
							if($result == "iperror")
							{
								...
								{
									ob_clean();
									header("Location: ".SITE_URL_EMPLOYEES."/pdf-login.php?error=3&".$checkingFailCaptcha."=".$checkingFailCaptchaCode);
									exit();
								}
								else
								{
									ob_clean();
									header("Location: ".SITE_URL_EMPLOYEES."/pdf-login.php?error=3");
									exit();
								}
							...
							else
							{
								...
								{
									ob_clean();
									header("Location: ".SITE_URL_EMPLOYEES."/pdf-login.php?error=2&".$checkingFailCaptcha."=".$checkingFailCaptchaCode);
									exit();
								}
								elseif($attemptedTimes > 6)
								{
								...
								{
									ob_clean();
									header("Location: ".SITE_URL_EMPLOYEES."/pdf-login.php?error=2");
									exit();
								}
							}
						}
					...
					else
					{
						...
						{
							ob_clean();
							header("Location: ".SITE_URL_EMPLOYEES."/pdf-login.php?error=1&".$checkingFailCaptcha."=".$checkingFailCaptchaCode);
							exit();
						}
						elseif($attemptedTimes == 6)
						{
						...
						{
							ob_clean();
							header("Location: ".SITE_URL_EMPLOYEES."/pdf-login.php?error=1");
							exit();
						}
					}
				}
