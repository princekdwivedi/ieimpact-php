...
// $worksheet1->write_number(3, 1, 1);
 ...
 $query	=	"SELECT members_orders.memberId,firstName,lastName,COUNT(orderId) AS totalCustomersOrders FROM members INNER JOIN members_orders ON members.memberId=members_orders.memberId ".$whereClause.$andClause." GROUP BY members_orders.memberId ORDER BY ".$orderBy;
  ...
  if(mysql_num_rows($result))
  {
	...
	while($row	=   mysql_fetch_assoc($result))
	{
		...
		//$memberTotalOrders		=	@mysql_result(dbQuery("SELECT COUNT(orderId) FROM members_orders WHERE memberId=$memberId"),0);
		...
		$memberTotalratedOrders		=	@mysql_result(dbQuery("SELECT COUNT(orderId) FROM members_orders WHERE memberId=$memberId AND rateGiven <> 0 AND isRateCountingEmployeeSide='yes'"),0);
		...
		$memberTotalRatesSum		=	@mysql_result(dbQuery("SELECT SUM(rateGiven) FROM members_orders WHERE memberId=$memberId AND rateGiven <> 0 AND isRateCountingEmployeeSide='yes'"),0);
		...
		$lastCalculateTotalratedOrders		=	@mysql_result(dbQuery("SELECT COUNT(orderId) FROM members_orders WHERE memberId=$memberId AND rateGiven <> 0 AND orderAddedOn >= '$calculateShowingOrderFrom' AND isRateCountingEmployeeSide='yes'"),0);
		...
		$worksheet1->write($i,0,$memberId);
		$worksheet1->write($i,1,$memberName);
		$worksheet1->write($i,2,$memberTotalOrders);
