<?php
	...
	if(isset($_GET['searchBy']))
	{
		...
		if($searchBy	== 1)
		{
			...
			$andClause1		=	" AND workedOnDate='$searchOn'";
  ...
  // HTTP headers
  HeaderingExcel('view-idle-MT-employees.xls');
...
// $worksheet1->write_number(3, 1, 1);
 ...
 // $worksheet1->write_formula(5, 4, "= SIN(PI()/2)");
  ...
  $query	=	"SELECT employee_details.employeeId,fullName FROM employee_details INNER JOIN employee_shift_rates ON employee_details.employeeId=employee_shift_rates.employeeId ".$whereClause.$andClause." ORDER BY firstName";
  ...
  if(mysql_num_rows($result))
  {
	...
	while($row			=	mysql_fetch_assoc($result))
	{
		...
		$query1			=	"SELECT workId FROM datewise_employee_works_money WHERE ID > ".MAX_SEARCH_MT_EMPLOYEE_WORKID." AND employeeId=$employeeId".$andClause1;
