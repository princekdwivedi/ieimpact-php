<?php
	...
	$searchBy			=	0;
	$sortingLink		=	"";
	$isSearchingEmployee=	0;
	$groupByClause		=	" GROUP BY employees_incentives_order_calculation.employeeId";

	$whereClause		=	"WHERE MONTH(incentiveDate)=$month AND YEAR(incentiveDate)= $year";
	$andClause			=	"";
	$queryString		=	"&searchBy=2&month=".$month."&year=".$year;
	$selectFields		=	"SUM(totalSingleDayIncentive) as totalIncentives,SUM(oneStar) as totalOneStar,SUM(twoStar) as totalTwoStar,SUM(threeStar) as totalThreeStar,SUM(fourStar) as totalFourStar,SUM(fiveStar) as totalFiveStar,fullName,employees_incentives_order_calculation.employeeId,incentiveDate";
	...
	if(isset($_GET['searchBy']))
	{
		...
		if($searchBy			==	1)
		{
			...
			$whereClause		=	"WHERE incentiveDate='$t_searchOn'";
			$queryString	    =	"&searchBy=1&searchOn=".$searchOn;
			$selectFields		=	"totalSingleDayIncentive as totalIncentives,oneStar as totalOneStar,twoStar as totalTwoStar,threeStar as totalThreeStar,fourStar as totalFourStar,fiveStar as totalFiveStar,fullName,employees_incentives_order_calculation.employeeId,incentiveDate";
		...
		if(isset($_GET['searchEmployee']))
		{
			...
			$searchEmployee		=	trim($searchEmployee);

			if(!empty($searchEmployee))
			{
				$andClause		 =	" AND fullName='$searchEmployee'";
...
<table width='98%' align='center' cellpadding='2' cellspacing='2' border='0'>
	...
	<tr>
		<td class="heading3">
			:: VIEW EMPLOYEES INCENTIVES ON ORDER RATINGS ::
		</td>
	</tr>
	<tr>
...
$().ready(function() {
	$("#searchName").autocomplete("<?php echo SITE_URL_EMPLOYEES?>/search-all-pdf-employee.php", {width: 230,selectFirst: false});
});

function redirectViewPageTo(flag,url)
{
	window.location.href="<?php echo SITE_URL_EMPLOYEES;?>/employee-incentives-on-ratings.php?"+flag+url;
}
</script>
<form name="searchIncentives" action="" method="GET">
	<table cellpadding="0" cellspacing="0" width='98%' align="center" border='0'>
		<tr>
			...
			<td width="12%" class="heading3" valign="top">
				Employee Name
			</td>
			<td width="15%" valign="top">
				<input type='text' name="searchEmployee" size="40" value="<?php echo $searchEmployee;?>" id="searchName">
			</td>
			<td valign="top">
...
<?php
	$start					  =	0;
	$recsPerPage	          =	200;	//	how many records per page
	$showPages		          =	10;	
	$pagingObj->recordNo	  =	$recNo;
	$pagingObj->startRow	  =	$recNo;
	$pagingObj->whereClause   =	$whereClause.$andClause.$groupByClause;
	...
	$pagingObj->selectColumns = $selectFields;
	if(!empty($groupByClause))
	{
		$pagingObj->primaryColumn = "employees_incentives_order_calculation.employeeId";
	}
	$pagingObj->path		  = SITE_URL_EMPLOYEES."/employee-incentives-on-ratings.php";
	...
	<table width="98%" border="0" cellpadding="0" cellspacing="0" align="center">
		<tr>
			<td class="heading3" width="20%"><b>Name</b></td>
			<td class="heading3" width="9%"><b>Date</b></td>
			<td class="heading3" width="4%" style="text-align:center"><b>Poor</b></td>
			<td class="heading3" width="7%" style="text-align:center"><b>Average</b></td>
			<td class="heading3" width="6%" style="text-align:center"><b>Good</b></td>
			<td class="heading3" width="8%" style="text-align:center"><b>Very Good</b></td>
			<td class="heading3" width="8%" style="text-align:center"><b>Excellent</b></td>
			<td class="heading3" style="text-align:center"><b>Incentives</b></td>
		...
		<?php
				while($row	=   mysql_fetch_assoc($recordSet))
				{
					...
					if($isSearchingEmployee	==	1)
					{
						$showDate		=	$incentiveDate;
					}
					else
					{
						if($searchBy		==	1)
						...
						{
							$showDate		=	$a_serachMonths[$month].",".$year;
						}
					}
					
		?>
		<tr>
			<td valign="top" width="smalltext2"><?php echo $fullName;?></td>
			<td valign="top" width="smalltext2"><?php echo $showDate;?></td>
