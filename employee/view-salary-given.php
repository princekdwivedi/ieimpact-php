<?php
	...
	if(isset($_GET['employeeId']) && isset($_GET['month']) && isset($_GET['year'])  && isset($_GET['departmentId']) && isset($_GET['salaryId']))
	{
		...
		if(!empty($employeeId) && !empty($month) && !empty($year) && !empty($departmentId) && !empty($salaryId))
		{
			$query				=	"SELECT * FROM employee_salary_given WHERE employeeId=$employeeId AND month=$month AND year=$year AND departmentId=$departmentId AND salaryId=$salaryId";
...
<center>
	...
	<table width="100%" border="0" cellpadding="0" cellspacing="0">
		...
		<tr>
			...
			<td class="textstyle1">
				<b><?php echo $totalLineFixedMoney;?></b>/-Rs.
			</td>
		</tr>
		<tr>
			<td height="5" colspan="3"></td>
