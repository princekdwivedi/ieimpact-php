<?php
	...
	$numberOfCompsFilled			=	$checkedCompsFiles;
	...
	if(mysqli_num_rows($result))
	{
		...
		while($row							=	mysqli_fetch_assoc($result))
		{
			...
			$a_orderAdminReplyMessages[$t_adminMessageId]	=	$t_adminMessageLevel;
		}
	...
	if(isset($_GET['orderId']) && isset($_GET['customerId']))
	{
		...
		$isManager		    =	$employeeObj->isEmployeeManager($orderAcceptedBy);
		$isExistsReply	    =	0;
		$query1 	        =	"SELECT * FROM members_orders_reply WHERE orderId=$orderId AND memberId=$customerId";
		$result1            =   dbQuery($query1);
		if(mysqli_num_rows($result1)){
			$row	        				=	mysqli_fetch_assoc($result1);
			$replyId       				    =   $isExistsReply = $row1['replyId'];
			$adminMessageId 				=   $row['replyAdminMessageId'];
...
<?php
	...
	if(isset($_REQUEST['formSubmitted']))
	{
		...
		{
			$replyAdminMessageId	=	$adminMessageId;
		}
		else
		{
		...
		{	
			...
			if(!empty($_FILES['replyOrderFile']['name']))
			{
				...
				if($isNewUploadingSystem == 1)
				{
					...
					$destFileName			=	$newUploadingPath."/".$replyId."_2_".$uploadingFileName.".".$ext;
			...
			if(!empty($_FILES['replyPublicRecordFile']['name']))
			{
				...
				if($isNewUploadingSystem == 1)
				{
					...
					$destFileName			=	$newUploadingPath."/".$replyId."_3_".$uploadingFileName.".".$ext;

					move_uploaded_file($tempName,$destFileName);

					dbQuery("INSERT INTO order_all_files SET uploadingType=2,uploadingFor=2,orderId=$orderId,memberId=$customerId,uploadingFileName='$t_uploadingFile',uploadingFileExt='$ext',uploadingFileType='$mimeType',uploadingFileSize=$fileSize,addedOn='".CURRENT_DATE_INDIA."',addedTime='".CURRENT_TIME_INDIA."',customerZoneDate='".CURRENT_DATE_CUSTOMER_ZONE."',customerZoneTime='".CURRENT_TIME_CUSTOMER_ZONE."',addedFromIp='".VISITOR_IP_ADDRESS."',replyOrderId=$replyId,excatFileNameInServer='$destFileName'");

					$hasReplyPublicRecordFile  = 1;
				}
			...
			if(!empty($_FILES['replyMlsFile']['name']))
			{
				...
				if($isNewUploadingSystem == 1)
				{
					...
					$destFileName			=	$newUploadingPath."/".$replyId."_4_".$uploadingFileName.".".$ext;
			...
			if(!empty($_FILES['otherFile']['name']))
			{
				...
				if($isNewUploadingSystem == 1)
				{
					...
					$destFileName			=	$newUploadingPath."/".$replyId."_5_".$uploadingFileName.".".$ext;
			...
			dbQuery("UPDATE members_orders_reply SET hasCompletedPdfFile=$hasCompletedPdfFile,orderReplyFileMd5HasSize='$orderReplyFileMd5HasSize',hasReplyOrderFile=$hasReplyOrderFile,hasReplyPublicRecordFile=$hasReplyPublicRecordFile,hasReplyMlsFile=$hasReplyMlsFile,hasOtherFile=$hasOtherFile WHERE replyId=$replyId");
			...
			ob_clean();
			header("Location: ".SITE_URL_EMPLOYEES ."/process-pdf-order-test.php?orderId=$orderId&customerId=$customerId&success=1&selectedTab=2");
			exit();
		}
		else
		{
