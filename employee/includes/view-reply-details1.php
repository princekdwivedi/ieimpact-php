<?php
	...
	if($result							=	$orderObj->getReplyOrderDetails($orderId,$customerId))
	{
		...
		{
			$completedMin		=	timeBetweenTwoTimes($orderPlacedDate,$orderAddedTime,$orderCompletedOn,$orderCompletedTime);
		...
		<td class="smalltext24">
			<?php
				if($isAddedTatTiming		==	1)
				{
					...
					echo showDateTimeFormat($orderCompletedOn,$orderCompletedTime)." ".$expctDelvText." ".$onTimeText.$postAuditText;
				}
				else
				{
					echo showDateTimeFormat($orderCompletedOn,$orderCompletedTime)." ".$expctDelvText.$postAuditText;
				}
			...
			<?php 
				if($hasCompletedPdfFile)
				{
						...
						if($result	=	$orderObj->getOrderMultipleFiles($orderId,$replyId,2,7))
						{
							...
							$isViewedDownloaded			=	$row['isViewedDownloaded'];
							$viewedDownloadedEstDate	=	$row['viewedDownloadedEstDate'];
							$viewedDownloadedEstTime	=	$row['viewedDownloadedEstTime'];
				...
				if($hasReplyOrderFile)
				{
					$repliedFName			   =  "";
						...
						if($result	=	$orderObj->getOrderMultipleFiles($orderId,$replyId,2,1))
						{
							...
							$repliedFName		=	$uploadingFileName.".".$uploadingFileExt;
