				...
				<?php
					...
					{	
						...
						if(isset($displayEmployeeTargetDone) && $displayEmployeeTargetDone == 1){	
							...
							$currentMonthExceedTat = $employeeObj->getSingleQueryResult("select count(*) as total from members_orders where orderId >= ".MAX_SEARCH_EMPLOYEE_ORDER_ID." and CONCAT(employeeWarningDate, ' ', employeeWarningTime) <= CONCAT(orderCompletedOn, ' ', completedTime) and MONTH(orderAddedOn)=$checkTargetCurrentMonth and YEAR(orderAddedOn)=$currentTargetYear and status IN (2,4,5)","total");

							if(empty($currentMonthExceedTat)){
								$currentMonthExceedTat 	=	 0;
							}

							$lastMonthExceedTat = $employeeObj->getSingleQueryResult("select count(*) as total from members_orders where orderId >= ".MAX_SEARCH_EMPLOYEE_ORDER_ID." and CONCAT(employeeWarningDate, ' ', employeeWarningTime) <= CONCAT(orderCompletedOn, ' ', completedTime) and MONTH(orderAddedOn)=$checkTargetLastMonth and YEAR(orderAddedOn)=$checkExceedTataLastYear and status IN (2,4,5)","total");
