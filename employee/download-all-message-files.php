<?php
	...
	if(isset($_GET[$M_D_5_ORDERID]) && isset($_GET[$M_D_5_ID]))
	{
		...
		$messageId				=	base64_decode($encodeMessageID);

		if(empty($orderId) || empty($messageId))
		{
			ob_clean();
	...
	function zipFilesAndDownload($file_names,$archive_file_name,$folderId,$orderAddress)
	{
		...
		if ($zip->open($archive_file_name,  ZipArchive::CREATE)) {
			...
			{
				$addFileNames	=	stringReplace("/home/ieimpact/WebFiles/files/orderFiles/".$folderId."//","",$files);
		...
		$downloadFileName		=	$orderAddress."-messages-files.zip";
