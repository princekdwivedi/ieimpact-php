<?php
    ...
    if(mysql_num_rows($result))
    {
		...
		while($row	=   mysql_fetch_assoc($result))
		{
			...
			$lastCalculateDaysOrders=	@mysql_result(dbQuery("SELECT COUNT(orderId) FROM members_orders WHERE memberId=$memberId AND acceptedBy=$s_employeeId AND orderAddedOn >= '$calculateShowingOrderFrom'"),0);
			...
			$calculateCustomerTotalProcessedOrders	  =	@mysql_result(dbQuery("SELECT COUNT(orderId) FROM members_orders_reply WHERE memberId=$memberId AND replyFileAddedOn  >= '$calculateShowingOrderFrom' AND orderId IN ($employeesAcceptedOrder)"),0);
			if(empty($calculateCustomerTotalProcessedOrders))
			{
				$calculateCustomerTotalProcessedOrders=	0;
			}
			
			$calculateCustomerTotalProcessedOrdersTime	=	@mysql_result(dbQuery("SELECT SUM(timeSpentEmployee) FROM members_orders_reply WHERE memberId=$memberId AND replyFileAddedOn  >= '$calculateShowingOrderFrom' AND orderId IN ($employeesAcceptedOrder)"),0);
...
<table width='98%' align='center' cellpadding='2' cellspacing='2' border='0' style="border:3px solid #4d4d4d;">
	...
	<tr>
		<td colspan='15' align="right">
			<img src="<?php echo SITE_URL;?>/images/close.gif" title="Close" border="0" onclick="javascript:removeEmployeesTotal(2)">&nbsp;
		</td>
	</tr>
</table>
</div>