<?php
	...
	if(!isset($_SESSION['hasManagerAccess']))
	{
		if(isset($_SESSION['searchCustomersForNonManager']))
		{
			...
			$serachEndClause	=	" AND members_orders.memberId IN ($searchCustomersForNonManager)";
		}
		else
		{
			$serachEndClause	=	"";
	...
	if(isset($_SESSION['employeeId'])){
		...
		$query	=	"SELECT firstName,lastName FROM members WHERE memberType='".CUSTOMERS."' AND (firstName LIKE '%$searchName%' OR lastName LIKE '%$searchName%') AND appraisalSoftwareType=1".$serachEndClause;
