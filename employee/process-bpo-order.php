<?php
	...
	if(isset($_GET['orderId']) && isset($_GET['customerId']))
	{
		...
		else
		{
			...
			else
			{
				...
				{
					if($s_employeeId != $orderAcceptedBy && !in_array($customerId,$a_qaCustomers))
					{
						ob_clean();
						header("Location: ".SITE_URL_EMPLOYEES."/new-pdf-work.php");
						exit();
	...
	$form		=	SITE_ROOT_EMPLOYEES."/forms/reply-bpo-order.php";
?>
...
<?php
	...
	if(isset($_REQUEST['formSubmitted']))
	{
		...
		$commentsToQa		=	addslashes($commentsToQa);
		if(empty($replyId))
		{
		...
		$validator ->checkField($timeSpentEmployee,"","Please enter total time spent to complete !!");
		...
		{	
			...
			if(empty($replyId))
			{
				$query	= "INSERT INTO members_orders_reply SET orderId=$orderId,memberId=$customerId,replyInstructions='$replyInstructions',hasRepliedFileUploaded=1,replyFileAddedOn='".CURRENT_DATE_INDIA."',replyFileAddedTime='".CURRENT_TIME_INDIA."'";
				dbQuery($query);
				...
				{
					dbQuery("UPDATE members_orders SET employeeId=$s_employeeId,acceptedBy=$s_employeeId,assignToEmployee='".CURRENT_DATE_INDIA."',assignToTime='".CURRENT_TIME_INDIA."' WHERE orderId=$orderId AND memberId=$customerId AND status=1");
			...
			dbQuery("UPDATE members_orders_reply SET commentsToQa='$commentsToQa',timeSpentEmployee=$timeSpentEmployee WHERE replyId=$replyId AND orderId=$orderId");
			if(!empty($_FILES['replyOrderFile']['name']))
			{
				...
				{
					$file	=	$orderId."_".$replyId."_".$replyOrderFileName.".".$replyOrderFileExt;
					chmod($replyOrderFilePath."$file",0644);
			...
			if(!empty($_FILES['replyPublicRecordFile']['name']))
			{
				...
				{
					$file1	=	$orderId."_".$replyId."_".$replyPublicRecordFileName.".".$replyPublicRecordFileExt;
					chmod($replyPublicRecordFilePath."$file1",0644);
			...
			ob_clean();
			header("Location: ".SITE_URL_EMPLOYEES ."/process-bpo-order.php?orderId=$orderId&customerId=$customerId&success=1");
			exit();
		}
	}
	include($form);
