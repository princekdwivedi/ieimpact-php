<?php
	...
	$whereClause				=   "WHERE employee_shift_rates.departmentId=1 AND isActive=1";
	$orderBy					=	"firstName";
	$queryString				=	"";
	...
	if(isset($_POST['formSubmitted']))
	{
		...
		ob_clean();
		header("Location: ".SITE_URL_EMPLOYEES."/daily-idle-employee.php?".$redirectLink);
		exit();
	}
	...
	if(isset($_GET['searchBy']))
	{
		...
		if($searchBy	== 1)
		{
			...
			$andClause1		=	" AND workedOnDate='$t_searchOn'";
			$text			=	"View idle MT employees on - ".showDate($t_searchOn);
		...
		else
		{
			$month			=	$_GET['month'];
			$year			=	$_GET['year'];
			$andClause1		=	" AND MONTH(workedOnDate)=$month AND YEAR(workedOnDate)=$year";
			...
			$text		    =	"View idle MT employees on - ".$monthText.",".$year;
	...
	<table cellpadding="0" cellspacing="0" width='98%'align="center" border='0'>
		<tr>
			<td colspan="15" class="textstyle1">
				<b>VIEW MT DAILY IDLE EMPLOYEE (Didn't do work or added lines)</b>
			</td>
		</tr>
		<tr>
...
<table cellpadding="0" cellspacing="0" width='98%'align="center" border='0'>
	...
	<tr>
		<td colspan="10">
			<a href="<?php echo SITE_URL_EMPLOYEES;?>/print-idle-mt-employee.php?<?php echo $printLink?>" class="link_style9">PRINT THIS REPORT IN EXCEL SHEET</a>
		</td>
	</tr>
	<tr>
		<td colspan="10">
	...
	<?php
		...
		while($row		=   mysql_fetch_assoc($result))
		{
			...
			$query1		=	"SELECT ID FROM datewise_employee_works_money WHERE ID > ".MAX_SEARCH_MT_EMPLOYEE_WORKID." AND employeeId=$employeeId".$andClause1;
	...
	<tr>
			...
			<td valign="top">
				<?php
					if($searchBy	==	1)
					{
						if($t_searchOn	< CURRENT_DATE_CUSTOMER_ZONE)
						{
					?>
					<a oncll="openEditWidow(<?php echo $employeeId;?>,'<?php echo $searchOn;?>')" class='link_style12' style="cursor:pointer;">Add Work</a>
					<?php
						}
						else
						{
